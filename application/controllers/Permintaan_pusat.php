...
class Permintaan_pusat extends CI_Controller
{
  ...
  public function index()
  {
    ...
    $this->load->view('permintaan_pusat/index', $data);
  ...
  public function add()
  {
    ...
    if ($validation->run()) {
      ...
      $this->db->insert('data_laporan_pusat', $data);
      redirect('permintaan-pusat');
    }
    // form
    $this->load->view('template/header');
    $this->load->view('template/sidebar', $data);
    $this->load->view('template/topbar', $data);
    $this->load->view('permintaan_pusat/add', $data);
    $this->load->view('template/footer');
  }
  ...
  public function delete($id = null)
  {
    ...
    // query
    if ($this->db->delete('data_laporan_pusat', ['id' => $id])) {
  ...
  public function kirim($laporan_id = null)
  {
    ...
    $data['dokumen'] = $this->db->get_where('ref_dokumen', ['jns' => 5])->result_array();
    // validasi
    $rules = [
      [
        'field' => 'status_minta',
    ...
    $this->load->view('template/sidebar', $data);
    $this->load->view('template/topbar', $data);
    $this->load->view('permintaan_pusat/kirim', $data);
    $this->load->view('template/footer');
  }
  ...
  public function upload($laporan_id = null, $dokumen_id = null)
  {
      ...
      if ($validation->run()) {
        ...
        if ($upload_file) {
          ...
          if ($this->upload->do_upload('file')) {
            ...
            redirect('permintaan-pusat/upload/' . $laporan_id . '/' . $dokumen_id . '');
          }
        }
        //query
        $data = [
        ...
        redirect('permintaan-pusat/kirim/' . $laporan_id . '');
      }
    }
    // form
    $this->load->view('template/header');
    $this->load->view('template/sidebar', $data);
    $this->load->view('template/topbar', $data);
    $this->load->view('permintaan_pusat/upload', $data);
    $this->load->view('template/footer');
  }
