        ...
        <div class="card">
          <form action="" method="post" autocomplete="off">
                    ...
                    foreach ($dokumen as $r) : ?>
                      <tr>
                        ...
                        <?php if ($upload) : ?>
                          ...
                          <td class="text-center"><a href="<?= base_url('permintaan-kanwil/delete-file/') . $laporan_id . '/' . $upload['id'] . '/' . $upload['file']; ?>" class="badge badge-danger" onclick="return confirm('Apakah Anda yakin akan menghapus file ini?');" data-toggle="tooltip" data-placement="bottom" title="Hapus"><i class="fa fa-trash"></i></a></td>
                        <?php else : ?>
                          <td></td>
                          <td class="text-center"><a href="<?= base_url('permintaan-kanwil/upload/') . $laporan_id . '/' . $dokumen_id; ?>" class="badge badge-info" data-toggle="tooltip" data-placement="bottom" title="Upload"><i class="fa fa-upload"></i></a></td>
                        <?php endif; ?>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
            ...
            <div class="card-footer">
              <?php
                                                        $dokumen = $this->db->get_where('ref_dokumen', ['jns' => 3])->num_rows();
                                                        $upload = $this->db->get_where('data_upload', ['laporan_id' => $laporan_id, 'jns' => 3])->num_rows();
              ?>
              ...
              <input type="hidden" name="status_minta" value="1">
            </div>
          </form>
        </div>
      </div>
