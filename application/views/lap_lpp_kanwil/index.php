            ...
            <div class="row">
              <div class="col">
                <?php foreach ($tahun as $t) : ?>
                  <a href="<?= base_url('lap-lpp-kanwil/index/') . $t['tahun'] . '/' . $bln; ?>" class="btn btn-sm btn-outline-success <?= $t['tahun'] == $thn ? 'active' : '' ?> ml-1 mt-1 mb-1"><?= $t['tahun']; ?></a>
                <?php endforeach; ?>
              </div>
            </div>
            <div class="row">
              <div class="col">
                <?php foreach ($bulan as $b) : ?>
                  <a href="<?= base_url('lap-lpp-kanwil/index/') . $thn . '/' . $b['bulan']; ?>" class="btn btn-sm btn-outline-success <?= $b['bulan'] == $bln ? 'active' : '' ?> ml-1 mt-1 mb-1"><?= $b['bulan']; ?></a>
                <?php endforeach; ?>
              </div>
            </div>
          </div>
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <?php if ($r['date_created_lpp']) : ?>
                          <a href="<?= base_url('lap-lpp-kanwil/delete/') . $thn . '/' . $bln . '/' . $r['id']; ?>" class="badge badge-danger" onclick="return confirm('Apakah Anda yakin akan mereset data ini?');" data-toggle="tooltip" data-placement="bottom" title="Reset"><i class="fa fa-trash"></i></a>
                        <?php endif; ?>
                      </td>
                    </tr>
                    <?php $j1 += $r['saldo_awal']; ?>
